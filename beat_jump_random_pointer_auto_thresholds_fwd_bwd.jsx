  ...
  */
  ...
  function get_even_pointers(start, end, pointers_total, index_offset) {
    ...
    for (var index = 0; index < pointers_total; index++) {
      ...
      var target_position = time + between - frameDur;
      pointers.push({
        number: index + index_offset,
        direction: direction,
        starting_position: starting_position,
        target_position: target_position,
        current_position: getRandomInRange(starting_position, target_position),
      })
    }
    return pointers;
  }
  ...
  for (var batch_start = 0; batch_start < work_frames; batch_start += frames_batch_size) {
    ...
    for (var index_in_batch = 0; index_in_batch < batch_length; index_in_batch++) {
      ...
      // var speed = input_C_value > window_stats.avg ? speed_max : speed_min;
      ...
      if (direction < 0 && starting_position > current_position) {
        current_position = starting_position + (starting_position - current_position);
        pointers[pointer_index].direction *= -1;
        times_pointers_reversed++;
      }
      ...
      if (FX_triggered) {
        ...
        if (prev_effect_index === 0) { // horizontal inversion
          ...
          pointers[pointer_index].current_position = getRandomInRange( // рандомайзим, чтобы не елозило по одному месту
            pointers[pointer_index].starting_position,
            pointers[pointer_index].target_position,
          );
